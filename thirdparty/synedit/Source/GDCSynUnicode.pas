  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {$ELSE}
        lpsz^ := WideChar(GDCSynUnicode.WCharUpper(PWideChar(lpsz^)));
        {$ENDIF}
        Inc(lpsz);
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {$ELSE}
        lpsz^ := WideChar(GDCSynUnicode.WCharLower(PWideChar(lpsz^)));
        {$ENDIF}
        Inc(lpsz);
      end;
    end;
...
begin
  ...
  if Len > 0 then
    {$IFDEF SYN_CLX} QSynUnicode. {$ELSE} GDCSynUnicode. {$ENDIF}
...
{$ELSE}
  ...
  begin
    ...
    if CanvasOrientation = coRightToLeft then
      Inc(X, GDCSynUnicode.TextWidth(ACanvas, Text) + 1);
    Windows.ExtTextOutW(Handle, X, Y, TextFlags, nil, PWideChar(Text),
     Length(Text), nil);
    MoveTo(X + GDCSynUnicode.TextWidth(ACanvas, Text), Y);
    Changed;
  end;
{$ENDIF}
end;
